...
const AccessoryBuilderResolver = require('./AccessoryBuilderResolver')
...
module.exports = (pluginName, platformName, homebridge) => {
  ...
  function ZWavePlatform (log, config, api) {
    ...
    this._accessoryBuilderResolver = new AccessoryBuilderResolver()
  ...
  ZWavePlatform.prototype._initAccessory = function (accessoryDisplayName, zwaveNode) {
    ...
    const ResolvedAccessoryBuilder = this._accessoryBuilderResolver.resolve(zwaveNode)

    if (!ResolvedAccessoryBuilder) {
      this._log.error(`${accessoryDisplayName} is currently unsupported`)

      return
    }

    const accessory = new ResolvedAccessoryBuilder(baseAccessoryBuilder, this._log, this._zwave)
