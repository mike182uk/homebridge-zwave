...
module.exports = (pluginName, platformName, homebridge) => {
  ...
  ZWavePlatform.prototype._initAccessory = function (accessoryConfig, zwaveNode) {
    ...
    let accessory, baseAccessoryBuilder

    // TODO: reduce baseAccessoryBuilder duplication
    switch (accessoryConfig.homekitAccessoryType) {
      case 'outlet':
        baseAccessoryBuilder = new BaseAccessoryBuilder(
          () => cachedAccessory || new homebridge.platformAccessory(accessoryDisplayName, accessoryUUID, homebridge.hap.Accessory.Categories.Outlet), // eslint-disable-line
      ...
      case 'sensor':
        baseAccessoryBuilder = new BaseAccessoryBuilder(
          () => cachedAccessory || new homebridge.platformAccessory(accessoryDisplayName, accessoryUUID, homebridge.hap.Accessory.Categories.Sensor), // eslint-disable-line
          homebridge.hap.Service,
          homebridge.hap.Characteristic
        )
