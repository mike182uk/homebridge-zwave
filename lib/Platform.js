...
module.exports = (pluginName, platformName, homebridge) => {
  ...
  class ZWavePlatform {
    ...
    constructor (log, config, api) {
      ...
      this._api.on('shutdown', this._sigintPlatform.bind(this))
    }
    ...
    _validateConfig () {
      const accessoryConfigSchema = Joi.object().keys({
        ...
        homekitCategory: Joi.string().valid(['Lightbulb', 'Sensor','DoubleSwitch']).required(),
        homekitServices: Joi.array()
          .when('homekitCategory', {
            is: 'Light',
            then: Joi.array().items(['Dimmable','NonDimmable','Color']).min(1).required()
          })
          .when('homekitCategory', {
            is: 'Sensor',
          ...
          .when('homekitCategory', {
            is: 'DoubleSwitch',
            then: Joi.array().items(['Switch']).min(1).required()
          })
      })
    ...
    _sigintPlatform () {
      ...
      this._log.info('Shutting down ZWave network...')

      this._zwave.sigint((err) => {
        if (err) {
          this._log.error(`An error occured shuttingdown the ZWave network: ${err.message}`)
          return
        }

        this._log.info('Platform Shutdown!')
      })
    }
