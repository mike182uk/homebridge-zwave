...
class AccessoryManager {
  ...
  _configureAccessoryInformationService (zwaveNode) {
    this._getService(this._hapService.AccessoryInformation)
      ...
      .setCharacteristic(this._hapCharacteristic.SerialNumber, 'Unknown')
  }
  ...
  _configureOutletService () {
    ...
    onCharacteristic
      ...
      .on('set', (value, done) => {
        ...
        const nodeValueId = `${zwaveNodeId}-${COMMAND_CLASS_SWITCH_BINARY}-1-${SWITCH_BINARY_INDEX_SWITCH}`
    ...
    const onCharacteristicZwaveNodeValueId = `${zwaveNodeId}-${COMMAND_CLASS_SWITCH_BINARY}-1-${SWITCH_BINARY_INDEX_SWITCH}`
    ...
    const outletInUseZwaveNodeValueId = `${zwaveNodeId}-${COMMAND_CLASS_METER}-1-${METER_INDEX_ELECTRIC_INSTANT_POWER}`
  ...
  _configureHumiditySensorService () {
    ...
    const zwaveNodeValueId = `${zwaveNodeId}-${COMMAND_CLASS_SENSOR_MULTILEVEL}-1-${SENSOR_MULTILEVEL_INDEX_HUMIDITY}`
  ...
  _configureLightSensorService () {
    ...
    // Setup handler for when the corresponding ZWave node value is updated outside of HomeKit
    const zwaveNodeValueId = `${zwaveNodeId}-${COMMAND_CLASS_SENSOR_MULTILEVEL}-1-${SENSOR_MULTILEVEL_INDEX_LUMINANCE}`
  ...
  _configureMotionSensorService () {
    ...
    // Setup handler for when the corresponding ZWave node value is updated outside of HomeKit
    const zwaveNodeValueId = `${zwaveNodeId}-${COMMAND_CLASS_ALARM}-1-${ALARM_INDEX_HOME_SECURITY}`
  ...
  _configureTemperatureSensorService () {
    ...
    // Setup handler for when the corresponding ZWave node value is updated outside of HomeKit
    const zwaveNodeValueId = `${zwaveNodeId}-${COMMAND_CLASS_SENSOR_MULTILEVEL}-1-${SENSOR_MULTILEVEL_INDEX_TEMPERATURE}`
