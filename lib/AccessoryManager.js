...
class AccessoryManager {
  ...
  _configureMotionSensorService () {
    ...
    const characteristic = service.getCharacteristic(this._hapCharacteristic.MotionDetected)

    // Setup handler for when the value for the "Motion Detected" characteristic is requested by HomeKit
    const characteristicName = 'Motion Detected'
    const zwaveNodeValueCriteria = {
      commandClass: COMMAND_CLASS_ALARM,
      valueIndex: ALARM_INDEX_HOME_SECURITY
    }
    const valueFilter = value => value.includes('Motion Detected')
  ...
  _configureBatteryService () {
    ...
    const chargingStateCharacteristicValueFilter = value => !value.includes('Battery power')
