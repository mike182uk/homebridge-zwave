...
class AccessoryManager {
  ...
  _configureBatteryService () {
    ...
    batteryLevelCharacteristic
      .on('get', done => {
        this._log(`${this._accessory.displayName} "Battery Level" characteristic value requested`)
    ...
    chargingStateCharacteristic
      .on('get', done => {
        this._log(`${this._accessory.displayName} "Charging State" characteristic value requested`)
        ...
        const isCharging = !value.includes('Battery power')

        done(null, isCharging)
      })
    ...
    statusLowBatteryCharacteristic
      .on('get', done => {
        this._log(`${this._accessory.displayName} "Status Low Battery" characteristic value requested`)
        ...
        const isCharging = !currentPowerMode.includes('Battery power')
        const lowBattery = !isCharging && (Number(batteryLevel) < Number(minBatteryLevel))

        done(null, lowBattery)
      })
    ...
    this._zwave.onNodeValueChanged(batteryLevelCharacteristicZwaveNodeValueId, value => {
      ...
      this._log(`${this._accessory.displayName} "Battery Level" characteristic value updated to ${value} outside of HomeKit`)
      ...
      const isCharging = !currentPowerMode.includes('Battery power')
      const lowBattery = !isCharging && (value < Number(minBatteryLevel))
      ...
      if (statusLowBatteryCharacteristic.value !== lowBattery) {
        ...
        this._log(`${this._accessory.displayName} "Status Low Battery" characteristic value updated to ${lowBattery} outside of HomeKit`)
      }
    })

    const chargingStateCharacteristicZwaveNodeValueId = this._zwave.generateNodeValueId({
      nodeId: zwaveNodeId,
      commandClass: COMMAND_CLASS_BATTERY,
      valueIndex: BATTERY_INDEX_LEVEL
    })

    this._zwave.onNodeValueChanged(chargingStateCharacteristicZwaveNodeValueId, value => {
      const isCharging = !value.includes('Battery power')

      batteryLevelCharacteristic.updateValue(isCharging)

      this._log(`${this._accessory.displayName} "Charging State" characteristic value updated to ${value} outside of HomeKit`)
    })
  }
