...
class ZWave {
   ...
   * @param {Function} log
   */
  constructor (devicePath, networkKey, log) {
  ...
  init (done) {
    ...
    if (typeof this.networkKey === 'string') {
      ozwConfig.NetworkKey = Buffer.from(this._networkKey)
    }

    const ozw = this._ozw = new OpenZWave(ozwConfig)
