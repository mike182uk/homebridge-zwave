...
class ZWave {
  ...
  onNodeValueChanged (nodeValueId, callback) {
    console.log(`onNodeValueChanged: ${nodeValueId}`)
  ...
  _ozwDriverReady (homeId) {
    ...
    //this._log(`Driver Ready for home ${homeId} ... Scanning will now start`)
  }
  /**
  ...
  _ozwNodeReady (nodeId, nodeData) {
    ...
    console.log(`Node[${nodeId}]: READY (${nodeData.manufacturer} ${nodeData.product})`)
  ...
  _ozwNodeValueAdded (nodeId, commandClass, value) {
    ...
    if (nodeId === MONITOR_NODE) {
      console.log(`Node[${nodeId}], [${commandClass}]: Value added: ${value.value_id} (${value.label})`)
    }

    //console.log(`Node[${nodeId}]: Value added: ${value.value_id} (${value.label})`)
  }
  ...
  _ozwNodeValueChanged (nodeId, commandClass, value) {
    ...
    if (nodeId === MONITOR_NODE) {
      console.log(`Node[${nodeId}], [${commandClass}]: Value Changed ${existingNodeValue.value_id} (${existingNodeValue.label}) : ${previousNodeValueValue} -> ${value.value}`)
    }
