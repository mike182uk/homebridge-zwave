...
class AccessoryConfigurer {
  ...
  _configureOutletService (accessory, zwaveNode) {
    ...
    const zwaveNodeValueSearchCriteria = {
    ...
    onCharacteristic
      .on('get', done => {
        const nodeValue = findNodeValue(zwaveNode.values, zwaveNodeValueSearchCriteria)
    ...
    // Setup for when the corresponding ZWave node value is updated outside of HomeKit
    const zwaveNodeValueId = `${zwaveNode.id}-${COMMAND_CLASS_SWITCH_BINARY}-1-0` // TODO: is this always going to be the node value id?
